  ...
  for( var index in pkg.latest.sniper.snippets){
    ...
    ps.push(new q.Promise(function(resolve,reject){
      repos.getContent(repoPath, function(err,data){
        ...
        self.searchForJSON(ps,pkg,name,data);
...
ghClient.prototype.searchForJSON = function(ps,pkg,name,data){
  var self = this;
  data.forEach(function(path){
    if(endsWith(path.name, ".json")){
      console.log(pkg.name + " has extra config: ... downloading");
      ps.push(self.querySnippetJSON(ps,pkg,name,path.path));
    }
  });
}

// requests the snippets/<snippet-name>.json (if availablie
ghClient.prototype.querySnippetJSON = function(ps,pkg,name,path){
  var repoPath = deepcopy(name);
  repoPath.path =  path;
