...
var ghClient = function(pkg, opts) {
  var self = this;
  this.log = opts.log;
...
ghClient.prototype.querySnippets = function(pkg, name) {
  ...
  if (pkg.latest.sniper.snippets == undefined) {
    pkg.latest.sniper.snippets = ["snippets", "examples"];
  }
  var ps = [];

  var downloadPkg = function(repoPath) {
    return new q.Promise(function(resolve, reject) {
      repos.getContent(repoPath, function(err, data) {
        // err is null
        if (err != undefined || data == undefined) {
          self.log.debug("error in querySnips", repoPath, err);
          // remove from snippets
          pkg.latest.sniper.snippets.splice(pkg.latest.sniper.snippets.indexOf(index));
          reject({
            name: "querySnippets",
            err: err
          });
  ...
  for (var index in pkg.latest.sniper.snippets) {
    ...
    self.log.info(pkg.name + ":snippets:", repoPath.user + repoPath.path);
    // snippet folder list
    ps.push(downloadPkg(repoPath));
  }

  // it is enough that we found one valid path
  return q.any(ps);
};
...
ghClient.prototype.searchForJSON = function(pkg, name, data) {
  ...
  data.forEach(function(path) {
    if (endsWith(path.name, ".json")) {
      self.log.info(pkg.name + " has extra config: ... downloading");
...
// deprecated
// @see: https://github.com/biojs/biojs/issues/96
...
var filterSnipData = function(data) {
  ...
  for (var key in data) {
    ...
    if (typeof data[key] === "undefined" || typeof data[key] == "string") {
      console.log("github not found for ", data);
      continue;
    }
...
ghClient.prototype.authenticate = function() {
  ...
  try {
    ...
    this.log.error("No Github creds provided");
    return false;
  }
